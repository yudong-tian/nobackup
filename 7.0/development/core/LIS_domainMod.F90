...
!TODO: clean this up with ntiles_pergrid array instead of maximum 
    ...
    do n=1,LIS_rc%nnest
!       LIS_ntiless(n,LIS_localPet) = LIS_rc%ntiles(n)
!       LIS_tdeltas(n,LIS_localPet) = LIS_rc%ntiles(n)

!       do m=1,LIS_rc%max_model_types
!          LIS_npatches(n,m,LIS_localPet)     = LIS_rc%npatch(n,m)
!          LIS_patch_deltas(n,m,LIS_localPet) = LIS_rc%npatch(n,m)
!       enddo

!       LIS_ngrids(n,LIS_localPet) = LIS_rc%ngrid(n)
!       LIS_gdeltas(n,LIS_localPet) = LIS_rc%ngrid(n)

!       do k=1,LIS_rc%ndas
!          LIS_odeltas(n,k,LIS_localPet) = LIS_rc%ngrid(n)*LIS_rc%nobtypes(k)
!       enddo
       ...
       do k=1,LIS_rc%ndas
          odeltas(k) = LIS_rc%ngrid(n)*LIS_rc%nobtypes(k)
       enddo
!-----------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------
       ...
       if(LIS_masterproc) then 
          ...
          do i=1,LIS_npes-1
             do k=1,LIS_rc%ndas
                LIS_ooffsets(n,k,i) = LIS_ooffsets(n,k,i-1)+LIS_odeltas(n,k,i-1)
             enddo
          enddo
       end if
       ...
       enddo
       do k=1,LIS_rc%ndas
          call MPI_BCAST(LIS_ooffsets(n,k,:), LIS_npes, MPI_INTEGER,0, &
               LIS_mpi_comm, ierr)
       enddo
#endif
       ...
       gridEnsDG = ESMF_DistGridCreate(minIndex=(/1,1/),&
            maxIndex=(/LIS_rc%glbngrid(n),LIS_rc%nensem(n)/),&
            deBlockList=deblklist,rc=status)
       call LIS_verify(status)
       ...
       LIS_ensOnGrid(n) = ESMF_GridCreate(name = "LIS Grid Ensemble Space",&
...
!EOP
    ...
    integer              :: nse_ind
     ...
     LIS_rc%lnc(n) = ewe_halo_ind-&
          ews_halo_ind+1
     LIS_rc%lnr(n) = nse_halo_ind-&
          nss_halo_ind+1

     LIS_rc%lnc_red(n)= ewe_ind-&
          ews_ind+1
     LIS_rc%lnr_red(n)= nse_ind-&
          nss_ind+1

     write(unit=LIS_logunit,fmt=*)'[INFO] local domain',':(',LIS_rc%lnc(n),LIS_rc%lnr(n),')'
     write(unit=LIS_logunit,fmt=*)'[INFO] local domain without halo',':(',&
          LIS_rc%lnc_red(n),LIS_rc%lnr_red(n),')'
!     write(unit=LIS_logunit,fmt=*)'parameter domain',':(',LIS_rc%pnc(n),LIS_rc%pnr(n),')'
     ...
     call MPI_ALLGATHER(ews_halo_ind,1,MPI_INTEGER,LIS_ews_halo_ind(n,:),1,MPI_INTEGER,&
          LIS_mpi_comm,ierr)
     call MPI_ALLGATHER(nss_halo_ind,1,MPI_INTEGER,LIS_nss_halo_ind(n,:),1,MPI_INTEGER,&
          LIS_mpi_comm,ierr)
     call MPI_ALLGATHER(ewe_halo_ind,1,MPI_INTEGER,LIS_ewe_halo_ind(n,:),1,MPI_INTEGER,&
          LIS_mpi_comm,ierr)
     call MPI_ALLGATHER(nse_halo_ind,1,MPI_INTEGER,LIS_nse_halo_ind(n,:),1,MPI_INTEGER,&
          LIS_mpi_comm,ierr)
#else
     LIS_deltas(n,:) = deltas
...
!EOP
    ...
    ips = -1
    do i=1, nc_raw+1
       call LIS_mpDecomp(i,j,1,nc_raw+1,1,nr_raw+1,LIS_rc%npesx, LIS_rc%npesy,Px,Py,P)
...
!-----------------------------------------------------------------------
       !YDT special treatment of landcover threshold (>0 changed to >=0) for clm4.5
       !YDT call calculate_domdistribution(n, LIS_rc%nsurfacetypes, &
       call calculate_domdistribution_clm45(n, LIS_rc%nsurfacetypes, &
...
!YDT: implement a special version for clm4.5
!BOP
!
! !ROUTINE: calculate_domdistribution_clm45
! \label{calculate_domdistribution_clm45}
!
! !REVISION HISTORY:
!  09 Sept 2004: Sujay Kumar ; Initial version
!
! !INTERFACE:
  subroutine calculate_domdistribution_clm45(n, ntypes, minp, maxt, fgrd)
! !USES:
    implicit none
    ...
    do r=1,LIS_rc%lnr(n)
       do c=1,LIS_rc%lnc(n) 
          ...
          do i=1,ntypes
             ...
             do j=1,ntypes
                if(fvt(j).gt.maxv)then
                   !YDT changed gt to ge for clm4.5
                   !YDT if(fgrd(c,r,j).gt.0) then
                   if(fgrd(c,r,j).ge.0) then
                      maxv=fvt(j)
                      t=j
                   endif
                endif
...
!----------------------------------------------------------------------
  ...
  end subroutine calculate_domdistribution_clm45
    ...
    do r=1,LIS_rc%lnr(n)     
       do c=1,LIS_rc%lnc(n)             
          do m=1,LIS_rc%nensem(n)             
             if(LIS_LMLC(n)%landmask(c,r).gt.0) then
...
!EOP
    ...
    do k=1,LIS_rc%nsurfacetypes       
       ...
       enddo
       !YDT changed .gt. to .ge. to include 0-area tiles as CLM4.5 does
       !YDT if(selected.and.landcover(k).gt.0) then 
       if(selected.and.landcover(k).ge.0) then 
...
!EOP
    ...
    do t=1,LIS_rc%nsurfacetypes        
       if(LIS_LMLC(n)%landmask(c,r).gt.0.and.&
           ! YDT changed .gt. to .ge. to include 0-area tiles as CLM4.5 does
          !YDT  LIS_LMLC(n)%landcover(c,r,t).gt.0.0.and.&
            LIS_LMLC(n)%landcover(c,r,t).ge.0.0.and.&
...
!EOP
    ...
    do t=1, LIS_rc%nsurfacetypes
       !YDT changed .gt. to .ge. to include 0-area tiles as CLM4.5 does
       !YDT if(LIS_LMLC(n)%landcover(c,r,t).gt.0.0.and.&
       if(LIS_LMLC(n)%landcover(c,r,t).ge.0.0.and.&
            isSurfaceTypeSelected(&
            nint(LIS_LMLC(n)%surfacetype(c,r,t)))) then 
          kk = kk + 1
          if(kk.eq.i) then 
...
! !INTERFACE: 
  ...
  use LIS_logMod,         only  : LIS_getNextUnitNumber, LIS_releaseUnitNumber  !YDT debug
   integer                    :: ftnx   !YDT
! 
! !DESCRIPTION: 
...
! _______________________________________________________
    ...
    if(LIS_rc%npesx*LIS_rc%npesy.ne.LIS_npes) then 
       write(unit=LIS_logunit,fmt=*) '[ERR] Layout does not match the number of processors...'
       write(unit=LIS_logunit,fmt=*) '[ERR] npex, npey, ',LIS_rc%npesx,'x',LIS_rc%npesy,'!=',LIS_npes
...
!YDT test 7/21/16, single-cpu run
  write(*, *) "Saving a copy of lat/lon data, LIS_rc%lnc(n)=", LIS_rc%lnc(n), " LIS_rc%lnr(n)=", LIS_rc%lnr(n)
  ftnx = LIS_getNextUnitNumber()
  open(ftnx, file="latlon.4gs4r", access="sequential", form="unformatted")
    write(ftnx) lat
    write(ftnx) lon
    write(ftnx) locallat
    write(ftnx) locallon
  call flush(ftnx) 
  call LIS_releaseUnitNumber(ftnx)
!YDT end of test
