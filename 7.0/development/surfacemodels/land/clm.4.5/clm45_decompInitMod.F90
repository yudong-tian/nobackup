...
!------------------------------------------------------------------------------
    ...
    if (clump_pproc > 0) then
       ...
       if (nclumps < npes) then
          write(LIS_logunit,*) 'decompInit_lnd(): Number of gridcell clumps= ',nclumps, &
               ' is less than the number of processes = ', npes
          call endrun()
       end if
    else
       write(LIS_logunit,*)'clump_pproc= ',clump_pproc,'  must be greater than 0'
       call endrun()
    end if
    ...
    if (ier /= 0) then
       write(LIS_logunit,*) 'decompInit_lnd(): allocation error for procinfo%cid'
       call endrun()
    endif
    ...
    if (ier /= 0) then
       write(LIS_logunit,*) 'decompInit_lnd(): allocation error for clumps'
       call endrun()
    end if
    clumps(:)%owner   = -1
    clumps(:)%ncells  = 0
    ...
    if (npes > numg) then
       write(LIS_logunit,*) 'decompInit_lnd(): Number of processes exceeds number ', &
            'of land grid cells',npes,numg
       call endrun()
    end if
    if (nclumps > numg) then
       write(LIS_logunit,*) 'decompInit_lnd(): Number of clumps exceeds number ', &
    ...
    if (masterproc) then
       write(LIS_logunit,*) ' decomp precompute numg,nclumps,seglen1,avg_seglen,nsegspc=', &
    ...
    lsize = end-beg+1
    gsize = lni * lnj
!YDT    call mct_gsMap_init(gsMap_lnd_gdc2glo, gindex, mpicom, comp_id, lsize, gsize)
    deallocate(gindex)

    ! Diagnostic output

    if (masterproc) then
       write(LIS_logunit,*)' Surface Grid Characteristics'
       write(LIS_logunit,*)'   longitude points               = ',lni
       write(LIS_logunit,*)'   latitude points                = ',lnj
       write(LIS_logunit,*)'   total number of land gridcells = ',numg
       write(LIS_logunit,*)' Decomposition Characteristics'
       write(LIS_logunit,*)'   clumps per process             = ',clump_pproc
       write(LIS_logunit,*)' gsMap Characteristics'
!YDT       write(LIS_logunit,*) '  lnd gsmap glo num of segs      = ',mct_gsMap_ngseg(gsMap_lnd_gdc2glo)
       write(LIS_logunit,*)
    end if
...
!------------------------------------------------------------------------------
    ...
    do n = 1,nclumps
       if (clumps(n)%ncells  /= allvecg(n,1) .or. &
          ...
          write(LIS_logunit,*) 'decompInit_glcp(): allvecg error ncells ',iam,n,clumps(n)%ncells ,allvecg(n,1)
          write(LIS_logunit,*) 'decompInit_glcp(): allvecg error lunits ',iam,n,clumps(n)%nlunits,allvecg(n,2)
          write(LIS_logunit,*) 'decompInit_glcp(): allvecg error ncols  ',iam,n,clumps(n)%ncols  ,allvecg(n,3)
...
!YDT    ng = mct_gsmap_gsize(gsmap_lnd_gdc2glo)
!YDT    allocate(arrayg(ng))
...
!YDT
       ...
       elseif (k == 4) then
          ...
          write(LIS_logunit,*) 'decompInit_glcp error in k ',k
          call endrun()
       endif
       ...
       do g = begg,endg
          if (count(g) <  1) then
             write(LIS_logunit,*) 'decompInit_glcp warning count g ',k,iam,g,count(g)
          endif
          do l = 1,count(g)
             i = i + 1
             if (i < beg .or. i > end) then
                write(LIS_logunit,*) 'decompInit_glcp error i ',i,beg,end
                call endrun()
             endif
       ...
       enddo
       if (i /= end) then
          write(LIS_logunit,*) 'decompInit_glcp error size ',i,beg,end
          call endrun()
       endif
       lsize = end-beg+1
       gsize = num
...
!YDT
       ...
       if (dbug > 1) then
          ...
          do l = beg,end
             if (tarr2(l) /= gindex(l)) then
                write(LIS_logunit,*) 'decompInit_glcp error tarr2 ',k,l,gindex(l),tarr2(l)
                call endrun()
             endif
          enddo
          ...
          if (masterproc) then
             if (tarr1(1) /= 1) then
                write(LIS_logunit,*) 'decompInit_glcp error tarr1 ',k,1,tarr1(1)
                call endrun()
             endif
             do l = 2,ntest
                if (tarr1(l)-tarr1(l-1) /= 1) then
                   write(LIS_logunit,*) 'decompInit_glcp error tarr1 ',k,l,tarr1(l-1),tarr1(l)
                   call endrun()
                endif
             enddo
          endif
          ...
          if (masterproc) then
             write(LIS_logunit,*) 'decompInit_glcp gsmap [l,c,p] test passes for ',k
          endif
          !--- end test section      
       end if
    ...
    if (masterproc) then
       ...
       write(LIS_logunit,*)'   longitude points          = ',lni
       write(LIS_logunit,*)'   latitude points           = ',lnj
       write(LIS_logunit,*)'   total number of gridcells = ',numg
       write(LIS_logunit,*)'   total number of landunits = ',numl
       write(LIS_logunit,*)'   total number of columns   = ',numc
       write(LIS_logunit,*)'   total number of pfts      = ',nump
       write(LIS_logunit,*)' Decomposition Characteristics'
       write(LIS_logunit,*)'   clumps per process        = ',clump_pproc
       write(LIS_logunit,*)' gsMap Characteristics'
!YDT
#if 0
       write(LIS_logunit,*) '  lnd gsmap glo num of segs = ',mct_gsMap_ngseg(gsMap_lnd_gdc2glo)
       write(LIS_logunit,*) '  gce gsmap glo num of segs = ',mct_gsMap_ngseg(gsMap_gce_gdc2glo)
       write(LIS_logunit,*) '  lun gsmap glo num of segs = ',mct_gsMap_ngseg(gsMap_lun_gdc2glo)
       write(LIS_logunit,*) '  col gsmap glo num of segs = ',mct_gsMap_ngseg(gsMap_col_gdc2glo)
       write(LIS_logunit,*) '  pft gsmap glo num of segs = ',mct_gsMap_ngseg(gsMap_pft_gdc2glo)
#endif
       write(LIS_logunit,*)
    end if
...
#endif
          write(LIS_logunit,*)'proc= ',pid,' nclumps = ',procinfo%nclumps
...
end module clm45_decompInitMod
