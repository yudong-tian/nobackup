module clm45_STATICEcosysdynMOD
...
! !USES:
    use clmtype
    use clm45_pftvarcon, only : noveg, nc3crop, nbrdlf_dcd_brl_shrub
!
! !ARGUMENTS:
    implicit none
...
!YDT
    ...
    do k=1,12   !! loop over months and read vegetated data
       ...
       call read_clm45_monthly_veg_to_local_g3d(1, mlai, 'MONTHLY_LAI', 'lsmpft', zsindex=0, tindex=k)
...
!YDT       call shr_scam_getCloseLatLon (ncid, scmlat, scmlon, closelat, closelon,&
!YDT            closelatidx, closelonidx)
!YDT    endif
    
    do k=1,2   !loop over months and read vegetated data

       call read_clm45_monthly_veg_to_local_g3d(1, mlai, 'MONTHLY_LAI', 'lsmpft', zsindex=0, tindex=months(k))
       call read_clm45_monthly_veg_to_local_g3d(1, msai, 'MONTHLY_SAI', 'lsmpft', zsindex=0, tindex=months(k))
       call read_clm45_monthly_veg_to_local_g3d(1, mhgtt, 'MONTHLY_HEIGHT_TOP', 'lsmpft', zsindex=0, tindex=months(k))
       call read_clm45_monthly_veg_to_local_g3d(1, mhgtb, 'MONTHLY_HEIGHT_BOT', 'lsmpft', zsindex=0, tindex=months(k))
...
!BOP
!
! !ROUTINE: read_clm45_monthly_veg_to_local_g3d
! \label{read_clm45_monthly_veg_to_local_g3d}
!
! !INTERFACE:
  subroutine read_clm45_monthly_veg_to_local_g3d(n, var, varname, zdimname, zsindex, tindex)
! !USES:
...
!EOP
   ...
   integer :: t, g, gid, lastg, g0, i, j, ic0, ir0, gid0, lastgrid  ! how many tiles up to last grid
    ...
    integer                    :: gnc_b, gnr_b
    integer                    :: ftn
    integer                    :: k, gindex, start(4), count(4) 
    logical                    :: file_exists
    real,      allocatable     :: globalvar(:, :, :)    ! contains 1 timestep 
    real,      allocatable     :: localvar(:, :, :)    ! contains 1 timestep 

   if ( .not. present(zsindex) ) zsindex = 1
   if ( .not. present(tindex) ) tindex = 1

    start = 1
    start(4) = tindex   ! get 1 time step 
          ...
          ios = nf90_get_var(ftn, varid, globalvar, start=start, count=count)
  ...
  end subroutine read_clm45_monthly_veg_to_local_g3d



end module clm45_STATICEcosysDynMod
