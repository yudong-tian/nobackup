...
! !USES:
  ...
  use LIS_historyMod,         only  : LIS_writevar_bin  !YDT debug
...
!EOP
  ...
  real    :: zw1,zw2
  integer           :: status
  integer           :: ftn, tmpindex     !YDT debug
...
!only the particular ensemble member is initialized. 
  do t = 1, LIS_rc%ntiles(n)
     ...
     swd(t) = zw1 * LIS_forc(n,findex)%metdata2(3,index1)
     if (swd(t) < 0) then
        write(LIS_logunit,*) '2 warning!!!  SW radiation is negative!!'
        write(LIS_logunit,*) 'sw=', swd(t), '... negative'
        write(LIS_logunit,*) 'gdas2=', LIS_forc(n,findex)%metdata2(3,index1)
        call LIS_endrun
     end if
     
     if (swd(t).gt.LIS_CONST_SOLAR) then
        swd(t)=LIS_forc(n,findex)%metdata2(3,index1)
     endif
  end do

!YDT test
!  write(*, *) "t    tile(t)%index          col        row           lon            lat "
!  write(*, *) "======================================================================"

  do t = 1, LIS_rc%ntiles(n)     
     index1 = LIS_domain(n)%tile(t)%index
     lwd(t) = LIS_forc(n,findex)%metdata2(4,index1)     
     ...
     pcp(t) = LIS_forc(n,findex)%metdata2(8,index1)    
     if ( pcp(t) < 0.0 ) then
        pcp(t) = 0.0
     endif

     cpcp(t) = LIS_forc(n,findex)%metdata2(9,index1)     
     tmp(t) = wt1 * LIS_forc(n,findex)%metdata1(1,index1) & 
          + wt2 *LIS_forc(n,findex)%metdata2(1,index1)
     q2(t) = wt1 * LIS_forc(n,findex)%metdata1(2,index1) & 
          + wt2 *LIS_forc(n,findex)%metdata2(2,index1)
     uwind(t) = wt1 * LIS_forc(n,findex)%metdata1(5,index1) & 
          + wt2 *LIS_forc(n,findex)%metdata2(5,index1)     
     vwind(t) = wt1 * LIS_forc(n,findex)%metdata1(6,index1) & 
          + wt2 *LIS_forc(n,findex)%metdata2(6,index1)     
     psurf(t) = wt1 * LIS_forc(n,findex)%metdata1(7,index1) & 
          + wt2 *LIS_forc(n,findex)%metdata2(7,index1)
!YDT test 
!     tmpindex = LIS_domain(n)%tile(t)%col + (LIS_domain(n)%tile(t)%row -1) * LIS_rc%lnc(n)
!     vwind(t) = LIS_domain(n)%lon(tmpindex)
!     psurf(t) = LIS_domain(n)%lat(tmpindex)
!     write(*, '(4I8, 2F10.2)') t, LIS_domain(n)%tile(t)%index, &
!               LIS_domain(n)%tile(t)%col, LIS_domain(n)%tile(t)%row, &
!               LIS_domain(n)%lon(tmpindex), LIS_domain(n)%lat(tmpindex) 
  enddo

!YDT debugging 5/24/2016: see what we got here for swd
  write(*, *) "Saving a copy of GDAS forcing ... LIS_rc%ntiles(n)=", LIS_rc%ntiles(n)
  ftn = LIS_getNextUnitNumber()
  !open(ftn, file="gdas_swd.1gdr4", access="sequential", form="unformatted")
  open(ftn, file="gdas_forcing.9gdr4", access="sequential", form="unformatted")
