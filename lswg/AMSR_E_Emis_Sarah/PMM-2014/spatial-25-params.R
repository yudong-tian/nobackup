...
for (ich in 1:nch) { 
 for (ir in 1:nr ) {
  for (ic in 1:nc ) {
   ...
   x=t( rbind(xtbV[1, ], xtbH[1, ], xtbV[5, ], xtbH[5, ]) ) 
   #x=t( rbind(xtbV[1, ], xtbH[1, ]) ) 
   #x=t(rbind(xtbV) )
   #x=t(rbind(xtbH) )
   #x=t(rbind(xmpdi) )
   #x=t(rbind(xtbV, xtbH) )
   #x=t(rbind(xtbV, xtbH, xtbV2, xtbH2) )
   #x=t(rbind(xtbV, xtbH, xtbV2, xtbH2, xmpdi) )
   #x=t(rbind(xtbV, xtbH, xtbV2, xtbH2, xmpdi, xmpdi2) )
   ...
   vx=t( rbind(vxtbV[1, ], vxtbH[1, ], vxtbV[5, ], vxtbH[5, ]) ) 
   #vx=t( rbind(vxtbV[1, ], vxtbH[1, ]) ) 
   #vx=t( rbind(vxtbV) ) 
   #vx=t( rbind(vxtbH) ) 
   #vx=t( rbind(vxmpdi) ) 
   #vx=t( rbind(vxtbV, vxtbH) ) 
   #vx=t( rbind(vxtbV, vxtbH, vxtbV2, vxtbH2) ) 
   #vx=t( rbind(vxtbV, vxtbH, vxtbV2, vxtbH2, vxmpdi) ) 
   #vx=t( rbind(vxtbV, vxtbH, vxtbV2, vxtbH2, vxmpdi, vxmpdi2) ) 
   #vx=t( rbind(vxmpdi[1, ], vxmpdi2[1, ]) ) 

   errs[ic, ir, ich]=sqrt( mean( (vems[ic, ir, ich, ]-predict(model, 
           list(x=vx)) )^2, na.rm=T ) )
  }
 }
}
...
dev.copy(postscript, "plot-spatial-25-parm-err.ps", horizontal=T)
dev.off()
