...
fit_plot <- function (xm, ym, vxm, vym, title) { 
   ...
   vx = vxm[!is.na(vxm)]
   vy = vym[!is.na(vym)]
   plot(xm, ym, ylim=c(0.8, 1), xlim=c(0.7, 1.0), main=title, xlab="10H emis", ylab="emis")
   ...
   text(0.70, c(1, 0.99, 0.98, 0.97), labels=ca, col="blue", adj=0)
...
dev.copy(postscript, "plot-eval-fits-H-to-10H.ps", horizontal=F)
dev.off()
...
dev.copy(postscript, "plot-eval-fits-V-to-10H.ps", horizontal=F)
dev.off()
