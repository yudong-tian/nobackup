...
tbs = data[, , 6:15, ]
# 25 predictors 
tbV=tbs[, , 0:4*2+1, ]    #11V, 19V, 24V, 37V, 89V
tbH=tbs[, , 1:5*2, ]      #11H, 19H, 24H, 37H, 89H
tbV2=tbV**2 
tbH2=tbH**2 
mpdi=(tbV-tbH)/(tbV+tbH) 
mpdi2=mpdi**2 
ssi=tbV[, , 2, ] - tbV[, , 4, ] # Tb19V - Tb37V  
...
vmpdi2=vmpdi**2 
vssi=vtbV[, , 2, ] - vtbV[, , 4, ] # Tb19V - Tb37V  
...
errs6=array(-9999.0, dim=c(nc, nr, nch))
errs7=array(-9999.0, dim=c(nc, nr, nch))
errs8=array(-9999.0, dim=c(nc, nr, nch))
...
for (ich in 1:nch) { 
 for (ir in 1:nr ) {
  for (ic in 1:nc ) {
   ...
   # method 6: single channel MPDI: 10G   and SSI (18V-37V) (2-predictor)
   x=t( rbind(xmpdi[1, ], xssi) ) 
   model = lm(yem ~ x)  
   vx=t( rbind(vxmpdi[1, ], vxssi) ) 
   errs6[ic, ir, ich]=sqrt( mean( (vyem-predict(model, list(x=vx)) )^2, na.rm=T ) )

   # method 7: single channel MPDI and its square: 10G   and SSI (18V-37V) (3-predictor)
   x=t( rbind(xmpdi[1, ], xmpdi2[1, ],  xssi) )
   model = lm(yem ~ x)
   vx=t( rbind(vxmpdi[1, ], vxmpdi2[1, ], vxssi) )
   errs7[ic, ir, ich]=sqrt( mean( (vyem-predict(model, list(x=vx)) )^2, na.rm=T ) )

   # method 8: single channel MPDI 10G   and SSI (18V-37V) and its square (3-predictor)
   x=t( rbind(xmpdi[1, ], xssi, xssi**2) )
   model = lm(yem ~ x)
   vx=t( rbind(vxmpdi[1, ], vxssi, vxssi**2) )
   errs8[ic, ir, ich]=sqrt( mean( (vyem-predict(model, list(x=vx)) )^2, na.rm=T ) )
...
plot_err(lon, lat, errs6*100, "M6")
plot_err(lon, lat, errs7*100, "M7")
plot_err(lon, lat, errs8*100, "M8")
...
cat("method 6: single channel MPDI at 10G and SSI (18V-27V) (2-predictor)\n")
cat("method 7: single channel MPDI at 10G, its sqaure,  and SSI (18V-27V) (3-predictor)\n")
cat("method 8: single channel MPDI at 10G, SSI (18V-27V) and its square (3-predictor)\n")
